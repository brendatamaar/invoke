...
export async function signAwsSigV4Request(
  request: RequestConfig,
  options: AwsSigV4SignOptions = {},
  ...
  for (const header of headers) {
    ...
    signingHeaders.set(key, [
      ...(signingHeaders.get(key) ?? []),
      header.value.trim().replace(/\s+/g, " "),
    ]);
  }
  ...
  const canonicalHeaders = signedHeaderKeys
    .map((key) => `${key}:${(signingHeaders.get(key) ?? []).join(",")}\n`)
    .join("");
  const signedHeaders = signedHeaderKeys.join(";");
  const canonicalRequest = [
    request.method,
    canonicalAwsPath(url),
    canonicalAwsQuery(url),
    canonicalHeaders,
    signedHeaders,
    payloadHash,
  ].join("\n");
