...
const requireExists = <A>(entity: string, id: string, effect: Effect.Effect<A | undefined, StorageError>) =>
  effect.pipe(
    Effect.flatMap((v) =>
      v === undefined
...
export const DexieCollectionStore = Layer.succeed(CollectionStore, {
  ...
  getById: (id) =>
    tryStorage("getById", () => db.collections.get(id)).pipe(
  ...
  update: (collection) =>
    requireExists("collection", collection.id, tryStorage("getById", () => db.collections.get(collection.id))).pipe(
      Effect.flatMap(() =>
  ...
  delete: (id) =>
    tryStorage("delete", () => collectionStorage.deleteCollection(db, id)),
})
...
export const DexieEnvironmentStore = Layer.succeed(EnvironmentStore, {
  ...
  getById: (id) =>
    tryStorage("getById", () => db.environments.get(id)).pipe(
      Effect.map(Option.fromNullable),
    ),

  save: (env) =>
    tryStorage("save", () => environmentStorage.saveEnvironment(db, env)),

  delete: (id) =>
    tryStorage("delete", () => environmentStorage.deleteEnvironment(db, id)),
})

export const DexieHistoryStore = Layer.succeed(HistoryStore, {
  list: (limit = 100) =>
    tryStorage("list", () => historyStorage.listHistory(db, limit)),

  add: (entry) =>
    tryStorage("add", () => historyStorage.addHistory(db, entry)),

  delete: (id) =>
    tryStorage("delete", () => historyStorage.deleteHistoryEntry(db, id)),

  clear: () =>
    tryStorage("clear", () => historyStorage.clearHistory(db)),
})
...
export const DexieFlowStore = Layer.succeed(FlowStore, {
  ...
  getById: (id) =>
    tryStorage("getById", () => db.flows.get(id)).pipe(
      Effect.map(Option.fromNullable),
    ),

  save: (flow) =>
    tryStorage("save", () => flowStorage.saveFlow(db, flow)),

  delete: (id) =>
    tryStorage("delete", () => flowStorage.deleteFlow(db, id)),
})
...
export const DexieRequestStore = Layer.succeed(RequestStore, {
  ...
  getById: (id) =>
    tryStorage("getById", () => db.requests.get(id)).pipe(
      Effect.map(Option.fromNullable),
    ),

  delete: (id) =>
    tryStorage("delete", () => collectionStorage.deleteRequest(db, id)),
})
