...
export function compareResponses(left: ExecuteResponse, right: ExecuteResponse): DiffResult {
  ...
  changes.push(...compareValues(headersToObject(left.headers), headersToObject(right.headers), "headers"));
  if (left.status !== right.status) changes.push({ type: "change", path: "status", oldValue: left.status, value: right.status });
  if (left.statusText !== right.statusText) {
    changes.push({ type: "change", path: "statusText", oldValue: left.statusText, value: right.statusText });
  }
...
function compareValues(left: unknown, right: unknown, path: string): DiffChange[] {
  ...
  if (Array.isArray(left) && Array.isArray(right)) {
    ...
    return Array.from({ length }).flatMap((_, index) => compareValueAt(left[index], right[index], `${path}[${index}]`));
  }

  if (isObject(left) && isObject(right)) {
    const keys = new Set([...Object.keys(left), ...Object.keys(right)]);
    return [...keys].flatMap((key) => compareValueAt(left[key], right[key], path ? `${path}.${key}` : key));
  }
...
function compareText(left: string, right: string): DiffChange[] {
  ...
  for (let index = 0; index < length; index += 1) {
    ...
    else if (value === undefined) changes.push({ type: "remove", path: `line ${index + 1}`, oldValue });
    else changes.push({ type: "change", path: `line ${index + 1}`, oldValue, value });
  }
  return changes;
}
