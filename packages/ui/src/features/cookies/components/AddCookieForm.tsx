...
const FIELD =
  "text-xs font-mono bg-[var(--surface-2)] border border-[var(--border)] rounded px-2 py-1 w-full outline-none focus:border-[var(--accent)] placeholder:text-[var(--text-3)]";
...
export function AddCookieForm({ onDone }: { onDone: () => void }) {
  ...
  return (
    <form
      ...
      className="px-4 py-3 border-b border-[var(--border)] bg-[var(--surface)] flex flex-col gap-2"
    >
      ...
      <div className="flex justify-end gap-2">
        <button type="button" onClick={onDone} className="btn text-xs py-1 px-3">
          Cancel
        </button>
