...
function TokenExpiryRow({ tokenExpiresAt }: { tokenExpiresAt?: number }) {
  ...
  const value = expired ? (
    <span className="text-[var(--danger)]">
      Expired ({new Date(tokenExpiresAt).toLocaleString()})
    </span>
  ) : (
...
export function AuthenticationSection({
  ...
  return (
    <Section title="Authentication" icon={AUTH_ICON} meta={authMeta}>
      <div className="overflow-hidden rounded-md border border-[var(--border)] bg-[var(--surface-2)]">
        ...
        {auth?.type === "oauth2" && auth.flow === "authorization_code" && (
          <>
            ...
            {auth.accessToken && <TokenExpiryRow tokenExpiresAt={auth.tokenExpiresAt} />}
          </>
        )}
      </div>
    </Section>
...
export function CookiesSection({
  ...
  return (
    <Section title="Cookies" icon={COOKIE_ICON} meta={cookiesMeta}>
      <div className="overflow-hidden rounded-md border border-[var(--border)] bg-[var(--surface-2)]">
...
export function RedirectsSection({ redirects }: { redirects: any[] }) {
  const redirectsMeta = useMemo(
    () => <RedirectsMeta count={redirects.length} />,
    [redirects.length],
  );
  return redirects.length === 0 ? null : (
    <Section title="Redirects" icon={ROUTE_ICON} meta={redirectsMeta}>
      <div className="overflow-hidden rounded-md border border-[var(--border)] bg-[var(--surface-2)]">
        {redirects.map((r) => (
          <div
            key={`${r.status}-${r.url}`}
...
export function TlsSection({ tls, firstCertificate }: { tls: any; firstCertificate: any }) {
  ...
  if (!tls) return null;
  return (
    <Section title="TLS" icon={LOCK_ICON} meta={tlsMeta}>
      <div className="overflow-hidden rounded-md border border-[var(--border)] bg-[var(--surface-2)]">
