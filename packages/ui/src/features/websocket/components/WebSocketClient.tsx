...
export function WebSocketClient() {
  const { websocketConnectionId, websocketLog, websocketState, set, addToast } =
    useStore();
  const msgRef = useRef("");
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto font-mono text-2xs p-2 flex flex-col gap-1">
        {websocketLog.map((entry) => (
          <div
            key={entry.id}
            className={`flex items-start gap-2 p-1.5 rounded ${entry.direction === "sent" ? "bg-blue-50" : "bg-[var(--surface-2)]"}`}
          >
            ...
            <span className="flex-1 break-all text-[var(--text-1)]">
              {entry.body}
            </span>
        ...
        {!websocketLog.length && (
          <p className="text-[var(--text-3)] text-center mt-8">No messages</p>
        )}
      </div>
