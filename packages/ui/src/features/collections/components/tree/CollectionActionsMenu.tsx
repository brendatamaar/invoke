    ...
    >
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 z-20 bg-[var(--surface)] border border-[var(--border)] rounded-md shadow-[var(--shadow-2)] py-1 min-w-[160px]">
          <CollectionMenuItem icon={<Plus size={12} />} label="New Request" onClick={onNewRequest} />
          <CollectionMenuItem icon={<FolderPlus size={12} />} label="New Folder" onClick={onNewFolder} />
          ...
          <CollectionMenuItem icon={<Download size={12} />} label="Export ZIP" onClick={onExportZip} />
          <CollectionMenuItem icon={<Download size={12} />} label="Export OpenAPI" onClick={onExportOpenApi} />
          <div className="h-px bg-[var(--border)] my-1" />
          <CollectionMenuItem icon={<Trash2 size={12} />} label="Delete" onClick={onDelete} danger />
        </div>
      )}
    </div>
  );
