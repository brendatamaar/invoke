...
// GraphQL Options panel

function GQLOptionField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2">
...
function GQLSectionTitle({ children }: { children: React.ReactNode }) {
  return (
    ...
    <div className="flex flex-col gap-3 p-3">
      ...
      <GQLOptionField label="HTTP Method">
        <Select
          ...
          onChange={(e) => setGraphqlRequest({ httpMethod: e.target.value as "POST" | "GET" })}
          size="xs"
      ...
      {useGet && (
        <p className="text-2xs text-[var(--text-3)] pl-[calc(theme(spacing.36)+theme(spacing.2))]">
          Query, variables, and operationName are sent as URL params. File uploads and batch mode are unavailable with GET.
        </p>
      )}
      ...
      <GQLOptionField label="Content-Type">
        <div className="flex flex-col gap-1">
          {([
            ...
            { value: "application/graphql+json", label: "application/graphql+json" },
            ...
            <label key={value} className="flex items-center gap-2 cursor-pointer select-none">
              <input
                ...
                disabled={useGet}
              />
              <span className={`text-xs font-mono ${useGet ? "text-[var(--text-3)]" : "text-[var(--text-1)]"}`}>{label}</span>
            </label>
          ))}
        </div>
      </GQLOptionField>
