...
function buildDraft(
  ...
  if (protocol === "graphql") { timeoutMs = graphqlRequest.timeoutMs; options = { ...graphqlRequest.options }; }
  else if (protocol === "websocket") { timeoutMs = websocketRequest.timeoutMs ?? 30000; options = { ...websocketRequest.options }; }
  else if (protocol === "grpc") { timeoutMs = grpcRequest.timeoutMs; options = { ...grpcRequest.options }; }
  else { timeoutMs = request.timeoutMs; options = { ...request.options }; }
...
export function SettingsPanel() {
  const {
    showSettings, set,
    request, setRequest,
    graphqlRequest, setGraphqlRequest,
    websocketRequest, setWebsocketRequest,
    grpcRequest, setGrpcRequest,
  ...
  const [draft, setDraft] = useState<Draft>(() =>
    buildDraft(protocol, request, graphqlRequest, websocketRequest, grpcRequest)
  );
  ...
  const opts = draft.options as {
    ...
    proxy?: { type: "http" | "socks5"; url: string; username?: string; password?: string };
  };
  ...
  function patchProxy(patch: object) {
    patchOptions({ proxy: { type: opts.proxy?.type ?? "http", url: opts.proxy?.url ?? "", ...opts.proxy, ...patch } });
  }
  ...
  function handleSave() {
    ...
    if (protocol === "graphql") setGraphqlRequest({ timeoutMs: draft.timeoutMs, options: draft.options });
    else if (protocol === "websocket") setWebsocketRequest({ timeoutMs: draft.timeoutMs, options: draft.options });
    else if (protocol === "grpc") setGrpcRequest({ timeoutMs: draft.timeoutMs, options: draft.options as never });
  ...
  const Row = ({ label, children }: { label: string; children: React.ReactNode }) => (
    <div className="flex items-center gap-3">
      <span className="text-xs text-[var(--text-2)] w-32 shrink-0">{label}</span>
      <div className="flex-1 flex items-center gap-2">{children}</div>
    </div>
  );

  const Section = ({ title }: { title: string }) => (
    <p className="text-2xs font-semibold text-[var(--text-3)] uppercase tracking-wider mb-3">{title}</p>
  );
    ...
    >
      ...
      >
            ...
            <Row label="Theme">
              <div className="flex items-center gap-2">
                ...
                <button
                  role="switch"
                  aria-checked={isDark}
                  onClick={() => setDraft((d) => ({ ...d, theme: isDark ? "light" : "dark" }))}
                ...
                <span className="text-xs text-[var(--text-2)] ml-1 capitalize">{draft.theme}</span>
              </div>
            </Row>
          </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              {(protocol === "rest" || protocol === "graphql") && (
                <Row label="Follow Redirects">
                  <input
                    ...
                    onChange={(e) => patchOptions({ followRedirects: e.target.checked })}
                    className="w-4 h-4"
                  />
                </Row>
              )}
              <Row label="Verify TLS">
                <input
                  ...
                  onChange={(e) => patchOptions({ verifySsl: e.target.checked })}
                  className="w-4 h-4"
                />
              </Row>
            </div>
          ...
          <div>
            ...
            <div className="flex gap-4">
              {[
                ...
                <div key={label} className="flex-1 bg-[var(--surface-2)] rounded-lg px-3 py-2 text-center">
                  <div className="text-sm font-semibold">{value}</div>
                  <div className="text-2xs text-[var(--text-3)] mt-0.5">{label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex items-center justify-end gap-2 px-4 py-3 border-t border-[var(--border)]">
          <button onClick={handleCancel} className="btn text-xs px-4 py-1.5">Cancel</button>
          <button onClick={handleSave} className="btn btn-primary text-xs px-4 py-1.5">Save</button>
        </div>
      </div>
    </div>
  );
