...
import { FlowRunner, type Flow, type FlowStep, type VariableScope } from "@invoke/core";
...
function makeStep(type: FlowStep["type"]): FlowStep {
  ...
  if (type === "request") return { id, type, name: "Request", request: { url: "", method: "GET", params: [], headers: [], bodyMode: "none", body: "", auth: { type: "none" }, timeoutMs: 30000 }, continueOnFailure: false };
  if (type === "delay") return { id, type, name: "Delay", delayMs: 1000 };
  if (type === "condition") return { id, type, name: "Condition", condition: { source: "status", expression: "", matcher: "equals", expected: "200" }, thenSteps: [] };
  return { id, type: "loop", name: "Loop", steps: [], count: 3, maxIterations: 100 };
}

function StepEditorPanel({ step, onChange, onRemove }: { step: FlowStep; onChange: (s: FlowStep) => void; onRemove: () => void }) {
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between">
        <span className={`text-2xs font-semibold uppercase tracking-wider px-2 py-1 rounded ${STEP_LABEL_COLORS[step.type]}`}>
          {step.type}
        </span>
        <button onClick={onRemove} className="flex items-center gap-1 text-xs text-[var(--text-3)] hover:text-[var(--danger)]">
        ...
        <>
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex gap-2">
              <Select
                ...
                onChange={(v) => onChange({ ...step, request: { ...step.request, method: v as any } })}
                size="xs"
              >
              ...
              <input
                ...
                onChange={(e) => onChange({ ...step, request: { ...step.request, url: e.target.value } })}
              />
            </div>
          </div>
          <label className="flex items-center gap-2 text-sm text-[var(--text-2)] cursor-pointer">
            <input
              ...
              onChange={(e) => onChange({ ...step, continueOnFailure: e.target.checked })}
            />
            Continue on failure
          </label>
        </>
      ...
      {step.type === "condition" && (
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex gap-2">
              <Select value={step.condition.source} onChange={(v) => onChange({ ...step, condition: { ...step.condition, source: v as any } })} size="xs">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex gap-2">
              <Select value={step.condition.matcher} onChange={(v) => onChange({ ...step, condition: { ...step.condition, matcher: v as any } })} size="xs">
                {["equals", "notEquals", "exists", "gt", "lt", "contains", "matches"].map((m) => (
                  <option key={m} value={m}>{m}</option>
                ))}
              </Select>
              <input
                ...
                onChange={(e) => onChange({ ...step, condition: { ...step.condition, expected: e.target.value } })}
              />
            </div>
          </div>
        </div>
      ...
      {step.type === "loop" && (
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-[var(--text-2)]">Count</label>
            <input
              type="number"
              className="input text-sm py-1.5 w-32"
              min={1}
              placeholder="∞"
              value={step.count ?? ""}
              onChange={(e) => onChange({ ...step, count: e.target.value ? Number(e.target.value) : undefined })}
            />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-[var(--text-2)]">Max iterations</label>
            <input
              type="number"
              className="input text-sm py-1.5 w-32"
              min={1}
              ...
              onChange={(e) => onChange({ ...step, maxIterations: e.target.value ? Number(e.target.value) : undefined })}
            />
          </div>
        </div>
      )}
...
function FlowModal({ flow, onClose }: { flow: Flow; onClose: () => void }) {
  const { set, addToast, environments, activeEnvironmentId, sessionVariables, flowRunning, flowLog, flowResult } = useStore();
  ...
  const [draft, setDraft] = useState<Flow>({ ...flow, steps: flow.steps ?? [] });
  ...
  const runFlow = async () => {
    if (!draft.steps.length) { addToast("warn", "Flow has no steps"); return; }
  ...
  const selectedStep = selectedIndex !== null ? draft.steps[selectedIndex] : null;
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-3.5 border-b border-[var(--border)] shrink-0">
          ...
          />
          <button onClick={handleClose} className="text-[var(--text-3)] hover:text-[var(--text-1)] p-1 rounded hover:bg-[var(--surface-2)]">
            <X size={14} />
          </button>
        </div>
                ...
                <div key={step.id} className="relative flex flex-col">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-2xs text-[var(--text-3)] uppercase tracking-wide">{step.type}</div>
                      <div className="text-xs text-[var(--text-1)] truncate font-medium">{step.name}</div>
                      {step.type === "request" && (
            ...
            <div className="p-3 border-t border-[var(--border)] shrink-0">
              {addingStep ? (
                <div className="flex flex-col gap-0.5">
                  ...
                  <button onClick={() => setAddingStep(false)} className="text-2xs text-[var(--text-3)] hover:text-[var(--text-1)] px-2 py-1 text-left mt-0.5">
                    Cancel
                  </button>
                </div>
              ) : (
        ...
        {(flowLog.length > 0 || flowResult) && (
          <div className="border-t border-[var(--border)] bg-[var(--surface-2)] shrink-0 max-h-36 overflow-y-auto px-4 py-3 font-mono text-2xs flex flex-col gap-0.5">
            {flowLog.map((line, i) => (
              <span key={i} className={line.startsWith("✓") ? "text-emerald-600" : line.startsWith("✗") ? "text-red-600" : "text-[var(--text-2)]"}>
                {line}
              </span>
            ))}
            {flowResult && (
              <div className={`mt-1 p-1.5 rounded text-xs font-sans ${flowResult.status === "passed" ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}>
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-[var(--border)] bg-[var(--surface-2)] shrink-0">
          <button onClick={handleClose} className="btn text-xs">Close</button>
          <button onClick={saveFlow} className="btn text-xs">Save</button>
          {flowRunning
            ? <button onClick={stopFlow} className="btn btn-danger text-xs flex items-center gap-1.5"><Square size={12} />Stop</button>
            : <button onClick={runFlow} className="btn btn-primary text-xs flex items-center gap-1.5"><Play size={12} />Run</button>
          }
        </div>
      </div>
    </div>
...
export function FlowPanel() {
  ...
  const openNew = () => setEditingFlow({ id: "", name: "New Flow", steps: [], createdAt: Date.now(), updatedAt: Date.now() });
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-3 py-2 border-b border-[var(--border)]">
        ...
        <button onClick={openNew} className="text-[var(--text-3)] hover:text-[var(--text-1)] p-0.5"><Plus size={13} /></button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!flows.length && <p className="p-4 text-xs text-[var(--text-3)] text-center">No flows yet</p>}
      </div>
