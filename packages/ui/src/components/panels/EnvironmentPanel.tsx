...
export function EnvironmentPanel() {
  ...
  const selectEnv = (id: string | undefined) => set({ activeEnvironmentId: id });

  const openDraft = (env?: Environment) => {
    set({ envDraft: env ? { ...env, variables: [...(env.variables ?? [])] } : { id: "", name: "New Environment", variables: [] } });
  };
  ...
  const saveDraft = async () => {
    ...
    try {
      if (envDraft.id) {
        await coreStore.environments.update(envDraft.id, envDraft);
      } else {
        await coreStore.environments.create(envDraft);
      }
      const envs = await coreStore.environments.list();
  ...
  const deleteEnv = async (id: string) => {
    ...
    try {
      await coreStore.environments.delete(id);
      const envs = await coreStore.environments.list();
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-3 py-2 border-b border-[var(--border)]">
        ...
        <button onClick={() => openDraft()} className="text-[var(--text-3)] hover:text-[var(--text-1)] p-0.5">
          <Plus size={13} />
        </button>
      </div>
        ...
        <div className="flex-1 overflow-y-auto py-1">
          {environments.map((env) => (
            ...
            >
              ...
              <button onClick={(e) => { e.stopPropagation(); openDraft(env); }} className="opacity-0 group-hover:opacity-100 text-[var(--text-3)] hover:text-[var(--accent)]">
              ...
              <button onClick={(e) => { e.stopPropagation(); deleteEnv(env.id); }} className="opacity-0 group-hover:opacity-100 text-[var(--text-3)] hover:text-[var(--danger)]">
                <X size={12} />
              </button>
            </div>
          ))}
...
function EnvEditor({ draft, onSave, onCancel }: { draft: Environment; onSave: () => void; onCancel: () => void }) {
  ...
  const setVar = (i: number, field: keyof KeyValue, value: string | boolean) => {
    const vars = (draft.variables ?? []).map((v, idx) => idx === i ? { ...v, [field]: value } : v);
    setDraft({ variables: vars });
  };
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {(draft.variables ?? []).map((v, i) => (
          <div key={i} className="flex items-center gap-0 border-b border-[var(--border)] last:border-0 hover:bg-[var(--surface-2)]">
            <input type="checkbox" checked={v.enabled !== false} onChange={(e) => setVar(i, "enabled", e.target.checked)} className="w-3.5 h-3.5 mx-2 accent-[var(--accent)]" />
            ...
            <div className="flex-1 flex items-center">
              ...
              />
              <button onClick={() => setMasked((s) => { const n = new Set(s); n.has(i) ? n.delete(i) : n.add(i); return n; })} className="px-1 text-[var(--text-3)]">
                {masked.has(i) ? <EyeOff size={11} /> : <Eye size={11} />}
              </button>
            </div>
            <button onClick={() => removeVar(i)} className="px-2 text-[var(--text-3)] hover:text-[var(--danger)]">×</button>
          </div>
        ))}
