...
function authToMetadata(auth: GrpcAuthInput | undefined): ServerHeaderInput[] {
  ...
  switch (auth.type) {
    ...
    case "basic":
      return [{ key: "authorization", value: `Basic ${Buffer.from(`${auth.username ?? ""}:${auth.password ?? ""}`).toString("base64")}` }];
    case "api-key":
      ...
      return [];
    case "oauth2":
      return auth.accessToken ? [{ key: "authorization", value: `Bearer ${auth.accessToken}` }] : [];
    default:
      return [];
  }
}
