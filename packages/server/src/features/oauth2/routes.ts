...
export function registerOAuth2Routes(app: Hono) {
  ...
  app.post(
    ...
    async (c) => {
      const input = c.req.valid("json") as OAuth2AuthCodeStartInput;
  ...
  app.get("/api/oauth2/callback", async (c) => {
    ...
    try {
      ...
      return c.html(
        `<!DOCTYPE html><html><head><title>Authorization Successful</title></head><body style="font-family:sans-serif;padding:2rem;text-align:center"><h2>âœ“ Authorization Successful</h2><p>You can close this window and return to invoke.</p><script>window.close();</script></body></html>`,
      );
    } catch (e) {
      oauth2Pending.set(state, {
        status: "error",
